...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError('');
    setLoading(true);
    clearTokens(); // remove any stale tokens before registering
    try {
      const data = await register(email, password);
      saveTokens(data.access, data.refresh);
      window.location.href = '/';
    } catch (err: unknown) {
      const msg = (err as { response?: { data?: { email?: string[]; password?: string[]; detail?: string } } })
        ?.response?.data;
  ...
  return (
    ...
    >
      <div className="flex flex-col items-center" style={{ width: '384px', gap: '24px' }}>
        ...
        {/* Cat illustration */}
        <img src="/cat.png" alt="Cat" style={{ width: '160px', height: 'auto' }} />
